...
export function buildExfilFirstExtortionScenario(
  ...
  const questions: ScenarioQuestion[] = [
    ...
    },
    {
      ...
      kind: "single",
      options: [
        { value: "detect_not_prevent", label: "The relevant Endpoint DLP rule was live and correctly detected 1,847 matches on labeled client data — but its configured response was to log and notify, not to block file access, so it never interrupted the archiving step" },
        { value: "policy_disabled", label: "The DLP policy was disabled and this event should not exist at all" },
        { value: "false_positive", label: "This is a false positive — 7z.exe is common IT software, so the match should be dismissed" },
        { value: "blocked_but_logged_wrong", label: "The policy did block the activity; ActionTaken: Audit is just a logging quirk and doesn't reflect what actually happened" },
      ],
    ...
    },
    {
      ...
      kind: "single",
      options: [
        { value: "exfil_extortion_ransomware_class", label: "As a ransomware-class incident: an exfiltration-only extortion operation, where the leverage is entirely the stolen data rather than encrypted systems — response should mirror a ransomware incident (scope, contain, assess breach-notification obligations), not a lesser data-loss ticket" },
        { value: "downgrade_no_encryption", label: "Downgrade it — without an encryptor there is no ransomware, so this is a standard data-loss incident that can follow normal DLP-violation triage timelines" },
        { value: "wait_for_encryption", label: "Hold at current priority and wait to see whether an encryptor deploys before treating this as ransomware, since encryption is what defines the category" },
        { value: "false_extortion", label: "Treat the extortion email as unverified and unrelated until the sender independently proves possession of the actual files" },
      ],
