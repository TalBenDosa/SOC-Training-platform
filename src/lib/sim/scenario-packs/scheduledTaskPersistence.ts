...
export function buildScheduledTaskPersistenceScenario(
  ...
  const questions: ScenarioQuestion[] = [
    ...
    },
    {
      ...
      kind: "single",
      options: [
        { value: "confirms_task_fired", label: "It confirms the scheduled task actually fired — the parent process is svchost.exe hosting the Schedule service, exactly what Windows uses to launch scheduled tasks" },
        { value: "different_malware", label: "It means a second, unrelated piece of malware is now also on the host" },
        { value: "process_injection", label: "It proves netfix_agent.exe injected itself into svchost.exe to hide" },
        { value: "irrelevant_detail", label: "The parent process is incidental and doesn't add anything beyond evt_stp_04" },
      ],
    ...
    },
    {
      ...
      kind: "single",
      options: [
        { value: "chain_of_custody", label: "The full chain: a script downloaded from an unmanaged forum site spawned the process that registered the task, targeting a binary the same script had just written to a user's AppData folder" },
        { value: "onlogon_trigger", label: "Using an onlogon trigger, which only malicious tasks ever use" },
        { value: "rl_highest", label: "Requesting /rl highest, which only administrators are capable of specifying" },
        { value: "task_name", label: "The task name 'NetFixOptimizer' is itself a known malicious string" },
      ],
