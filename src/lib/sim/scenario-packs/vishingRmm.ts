  ...
  const events: TelemetryEvent[] = [
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
      ...
      raw: {
        ...
        "url.full": `https://${downloadHost}/AnyDesk.exe`,
        "url.domain": downloadHost,
        "url.path": "/AnyDesk.exe",
        "url.category": "Remote Access Tools",
        "http.request.method": "GET",
        "http.response.status_code": 200,
        "http.user_agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.2592.87",
        "network.application": "AnyDesk",
        "network.bytes": 5242880,
        "destination.domain": downloadHost,
        "destination.port": "443",
        "source.ip": victim.ip,
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
      ...
      raw: {
        ...
        "url.domain": relayHost,
        "url.category": "Remote Access Tools",
        "destination.domain": relayHost,
        "destination.ip": relayIp,
        "destination.port": "443",
        "network.application": "AnyDesk",
        "network.transport": "tcp",
        "network.protocol": "ssl",
        "network.bytes": 184320,
        "session.bytes": 184320,
        "source.ip": victim.ip,
        "user.name": victim.sam,
        "user.email": victim.email,
        "event.action": "tunnel-session",
      },
    },
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
      ...
      raw: {
        "zscaler.action": "Allowed",
        "url.full": stagingUrl,
        "url.domain": stagingIp,
        "url.path": "/support/qb-agent.msi",
        "url.category": "Miscellaneous or Unknown",
        "http.request.method": "GET",
        "http.response.status_code": 200,
        "http.user_agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) WindowsPowerShell/5.1.19041.4046",
        "destination.ip": stagingIp,
        "destination.port": "80",
        "network.bytes": 2734080,
        "source.ip": victim.ip,
        "user.name": victim.sam,
        "event.action": "web-request",
      },
