...
export function buildClipboardClipperScenario(
  ...
  const questions: ScenarioQuestion[] = [
    ...
    },
    {
      ...
      kind: "single",
      options: [
        { value: "interpreter_signal", label: "A command interpreter carrying out persistence is a distinct, higher-signal event from an installer's own file operations — it is the T1059.003 technique in its own right" },
        { value: "no_difference", label: "It makes no difference — both methods write the same registry value and are equally invisible" },
        { value: "cmd_malicious", label: "cmd.exe itself is malicious here, since legitimate installers never call it" },
        { value: "elevation_proof", label: "Spawning cmd.exe proves the installer escalated privileges beyond what the user granted" },
      ],
