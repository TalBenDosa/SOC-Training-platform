...
This is called the **detect → learn → improve** loop, and CTI is the fuel that drives it. Without good threat intelligence, your detection rules only catch attacks you have already seen. With good CTI, you can detect attacks before they reach your organisation.`,
    ...
    // ── Question 4 ──────────────────────────────────────────────────────────
    {
      ...
      options: [
        "A commercial endpoint detection and response tool sold by Microsoft that blocks malware directly on the endpoint using signature and behavioural analysis",
        ...
        "A network vulnerability scanner that finds open ports and misconfigured services on company servers, similar to Nessus or OpenVAS",
        "A centralised firewall management console used to push malicious-IP block rules out to perimeter firewalls across an organisation",
      ],
      answer: 1,
      explanation: "MISP (Malware Information Sharing Platform) is an open-source platform for structured threat intelligence sharing. Organisations create 'events' containing 'attributes' (IOCs like IPs, hashes, domains) and share them with trusted partner organisations, ISACs, and CERTs. It is widely used by national CERTs, financial institutions, and government agencies worldwide. It is NOT an endpoint detection tool like Microsoft Defender, a vulnerability scanner like Nessus, or a firewall management console — MISP's job is sharing intelligence between organisations, not enforcing controls on a single network.",
      xp: 30,
    },
...
Only after exhausting passive research would an analyst consider any active step.`,
      ...
      questions: [
        {
          ...
          options: [
            "A Microsoft Windows Update content-delivery server — this kind of outbound connection is expected and completely benign for any managed corporate laptop",
            ...
            "The company's own internal file server — this is a routine, expected internal network connection with no external component at all",
            "A standard public DNS resolver — this is simply a normal DNS lookup being carried out over port 443 using DNS-over-HTTPS",
          ],
          answer: 1,
    ...
    // ── Question 4 ──────────────────────────────────────────────────────────
    {
      ...
      options: [
        "Passive OSINT always requires an expensive commercial subscription like Recorded Future or Mandiant Advantage, while active OSINT relies entirely on free command-line tools such as nmap",
        ...
        "Passive OSINT is a technique reserved for government intelligence agencies with legal search authority, while active OSINT is the standard approach used by private-sector security firms",
        "There is no meaningful technical difference between the two — both categories generate identical network traffic and carry exactly the same level of exposure risk to the investigating analyst",
      ],
      answer: 1,
...
Break chain of custody, and the evidence may be inadmissible in court.`,
    ...
    // ── Question 2 ──────────────────────────────────────────────────────────
    {
      ...
      options: [
        "There is no problem at all — a 30-day backup window is a widely accepted industry standard, so you can safely restore from any backup created within that month without further checks",
        ...
        "The only real issue is operational: 30-day-old backups simply take longer to restore than a fresher 7-day backup, so plan for extra downtime during the restoration window",
        "Backup age is irrelevant to recovery planning — the only thing that matters is restoring from whichever backup is most recent at the time the restoration begins",
      ],
      answer: 1,
      ...
      questions: [
        {
          ...
          options: [
            "Legitimate svchost.exe instances routinely run from C:\\Windows\\Temp\\ during Windows Update cycles — this is expected, well-documented Microsoft behaviour",
            ...
            "The file's location on disk does not matter for determining legitimacy — only the process name itself, svchost.exe, is what an analyst should ever check",
            "C:\\Windows\\Temp\\ is a protected, write-restricted Windows system folder that ordinary malware has no ability to write files into under any circumstances",
          ],
          answer: 1,
        ...
        },
        {
          ...
          options: [
            "The ransomware activity is fully contained to the local C: drive of SRV-FILE01 only — the D: drive and network share fields are informational and show no actual spread",
            ...
            "The three network shares were merely browsed or accessed for enumeration purposes but were never actually encrypted, so no further containment action is needed at this time",
            "This access pattern is completely normal file server activity — production servers routinely enumerate and access network shares as part of their everyday scheduled operations",
          ],
          answer: 1,
...
A "medium" severity alert on a domain controller with a service account is higher priority than a "high" severity alert on a decommissioned test server. Context is everything.`,
      checkpoint: {
        ...
        options: [
          "An alert that fired incorrectly because the underlying detection rule itself is broken and needs to be rewritten from scratch",
          "An alert that fired correctly on real activity that is legitimate and authorised, like an IT admin's approved port scan",
          "An alert where the available evidence is too thin or ambiguous for the analyst to make any confident determination at all",
          "An alert that, regardless of its content, must always be escalated straight to a Tier-2 analyst without exception",
        ],
        answer: 1,
        explanation: "A Benign True Positive means the rule worked correctly and the event really happened as described — but the activity itself was authorised and legitimate, like an IT admin's approved inventory scan. That is different from a broken rule (False Positive), from insufficient evidence (Indeterminate), and BTPs are closed with documentation, not automatically escalated.",
      },
    },
...
Good documentation protects you, helps your colleagues understand past alerts, and builds the institutional knowledge that makes the whole SOC better over time.`,
    ...
    // ── Question 1 ──────────────────────────────────────────────────────────
    {
      ...
      options: [
        "Open and fully investigate each of the 100 alerts individually, strictly in the chronological order they arrived, before drawing any conclusion about the host",
        "Close all 100 alerts immediately without review, since a high volume of alerts from a single host is always, by definition, a false positive pattern",
        ...
        "Escalate all 100 alerts to Tier-2 immediately as a bulk ticket, without personally reviewing the content or pattern of any of them first",
      ],
      answer: 2,
      ...
      questions: [
        {
          ...
          options: [
            ...
            "The presence of Windows event code 4625 by itself is diagnostic, since this specific event code only ever appears in logs during an active cyberattack and never during any legitimate, everyday failed logon",
            "SubStatus 0xC000006A in this event means the target account has been placed into a locked-out state, and that particular SubStatus value only ever occurs during pre-authorised internal security testing",
            "LogonType 3 network logons are exclusively used by domain administrators carrying out scheduled maintenance work, so any event carrying this LogonType must represent an IT team running tests",
          ],
          answer: 0,
        ...
        },
        {
          ...
          options: [
            "False Positive — close the alert without escalation, since this is most likely an IT admin somewhere in the organisation who simply forgot their own password",
            "Benign True Positive — close with documentation only, noting that the brute-force detection rule fired exactly as expected and requires no further action",
            ...
            "Indeterminate — take no containment action and wait roughly 24 hours to observe whether the attacker eventually succeeds before deciding what to do",
          ],
          answer: 2,
    ...
    // ── Question 4 ──────────────────────────────────────────────────────────
    {
      ...
      options: [
        "Just the alert ID and the two words 'closed as FP' — nothing else is ever required for a properly closed alert",
        ...
        "The name and contact details of your shift supervisor, so they alone can be reached if anyone later questions the closure decision",
        "A full copy of any sensitive customer financial data that happened to appear in the alert, kept on file purely for future reference",
      ],
      answer: 1,
