  ...
  tasks: [
    ...
    // ── Reading 2: CVSS structure and bands ──────────────────────────────
    {
      ...
      content:
        ...
        "**One version note, because you will meet both in the wild.** Everything above describes **CVSS v3.1**, still the version most scanners and the U.S. National Vulnerability Database (NVD) report today, so it is the one to learn first. But FIRST published **CVSS v4.0** in November 2023, and you will increasingly see its vector strings (they begin with the prefix CVSS:4.0 instead of CVSS:3.1). The headline changes are worth recognising so a v4.0 vector does not confuse you: Attack Complexity was split into two separate metrics — Attack Complexity (AC) plus a new Attack Requirements (AT) that captures conditions outside the attacker's control; User Interaction gained a middle setting (None / Passive / Active, instead of just None / Required); and the frequently-misused Scope metric was removed entirely and replaced by two distinct impact sets — Vulnerable System impact (VC/VI/VA) for the component itself and Subsequent System impact (SC/SI/SA) for everything beyond it. The three-layer idea you just learned still holds, but v4.0 renames the Temporal group to Threat and adds an optional Supplemental group; always read the version prefix on a vector before assuming which metric letters apply.",
      codeExample:
