  ...
  // ─────────────────────────────────────────────
  {
    ...
    tasks: [
      // ── Reading 1 ──
      {
        ...
        checkpoint: {
          question: "According to the reading, which header field shows the real address where bounce messages go, and can reveal a mismatch with the claimed From address?",
          options: ["Return-Path", "Message-ID", "X-Originating-IP", "Received"],
          answer: 0,
          explanation:
            "Return-Path is where bounce messages actually go. If it points somewhere unrelated to the claimed From domain, that's a major red flag — the sender wants replies routed elsewhere.",
        },
      },
      ...
      // ── Reading 2 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "A homograph attack uses visually identical Unicode characters (like Cyrillic 'а' instead of Latin 'a') so the domain looks correct to the human eye while actually resolving somewhere else entirely — checking the Punycode representation reveals the trick.",
        },
      },
    ...
    tasks: [
      // ── Reading 1 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "With split tunnelling, only traffic destined for corporate systems passes through the encrypted tunnel and its security controls — everything else (personal browsing, downloads) goes straight to the internet with zero visibility for the SOC.",
        },
      },
      ...
      // ── Reading 2 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "Credential stuffing targets many different usernames from one IP using stolen username/password pairs from a breach. Classic brute force instead targets one username repeatedly from the same IP.",
        },
      },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    tasks: [
      // ── Reading 1 ──
      {
        ...
        checkpoint: {
          question: "According to the reading, what is the difference between a firewall 'deny' action and a 'drop' action?",
          options: [
            "They are identical; different vendors just use different words",
            "Deny/Block sends a response to the sender, while Drop silently discards the traffic with no response — useful against scanners",
            "Deny only applies to inbound traffic, Drop only applies to outbound traffic",
            "Drop permanently bans the source IP, while Deny is temporary",
          ],
          answer: 1,
          explanation:
            "Deny/Block actively sends a response back to the sender (e.g., a TCP reset), while Drop silently discards the packet with no reply at all — which is useful because it doesn't confirm to a scanner that a host even exists at that address.",
        },
      },
      ...
      // ── Reading 2 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "Beacons 'call home' at regular intervals with small, consistent payloads — the malware checking in for instructions. This regularity, often over a long-lived pattern, is the key signature that distinguishes beaconing from normal traffic.",
        },
      },
    ...
    tasks: [
      // ── Reading 1 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "Network-level DNS logs only show which machine's IP made a query. Sysmon Event ID 22 additionally captures the Image field — the exact executable path of the process that issued the query — answering 'which program is calling home to this domain?'",
        },
      },
      ...
      // ── Reading 2 ──
      {
        ...
        checkpoint: {
          ...
          answer: 1,
          explanation:
            "DGA malware tries many algorithmically generated domains, most of which are unregistered and return NXDOMAIN, until it finds the one the attacker actually registered that day — producing a burst of failed lookups known as an NXDOMAIN storm.",
        },
      },
