  ...
  tasks: [
    ...
    // ── Reading 7: Other OWASP LLM risks ───────────────────────────────────
    {
      ...
      content:
        ...
        "**LLM05:2025 — Improper Output Handling.** This is not about what the model SAYS, but about what your application DOES with what it says without checking it first. If an application takes an LLM's output and passes it directly into a system shell, a SQL query, or renders it as raw HTML in a browser, an attacker who can influence the model's output (often via prompt injection) can achieve classic injection-style exploitation — command injection, SQL injection, or stored XSS — with the LLM acting as an unwitting intermediary. Worked example: a coding-assistant chatbot that will happily explain how to write a shell command, if its output is piped straight into a terminal by an over-eager application, becomes a code-execution primitive.\n\n" +
        "**LLM06:2025 — Excessive Agency.** An LLM-based agent that has been granted more permissions, more tool access, or more autonomy than the specific task requires. The AI ticket-triage example from the indirect-injection reading is a case of excessive agency being the thing that turned a mere prompt-injection ATTEMPT into an actual DATA EXFILTRATION incident — the injection was the entry point, but excessive agency (an agent allowed to send arbitrary outbound email) was what made it damaging. The control is the same principle of least privilege this platform already teaches for human accounts and service accounts, applied to agents: scope each tool grant as narrowly as the task allows, and require human approval for any high-impact action.\n\n" +
        ...
        "**LLM10:2025 — Unbounded Consumption.** Covered in the model-theft reading as the resource-abuse pathway; it also covers plain denial-of-service via resource exhaustion (flooding an LLM application with expensive, long-context requests to run up compute cost or degrade availability for legitimate users) and 'denial of wallet' attacks against pay-per-token deployments.",
      checkpoint: {
    ...
    // ── Matching 1: OWASP LLM ID <-> description ───────────────────────────
    {
      ...
      pairs: [
        ...
        { id: "p5", left: "LLM06:2025 Excessive Agency", right: "An agent is granted more tool access or autonomy than its task requires, turning a benign attempt into real-world impact" },
        { id: "p6", left: "LLM10:2025 Unbounded Consumption", right: "No meaningful limit on a single identity's query volume or cost, enabling resource-exhaustion, denial-of-wallet, or model extraction" },
      ],
      explanation:
        "These six cover the categories this room went into real depth on: prompt injection as the entry point, sensitive disclosure and poisoning as the two ways a model's OWN data can hurt you, supply chain as the upstream version of poisoning, excessive agency as the multiplier that turns an attempt into an incident, and unbounded consumption as the resource-abuse pathway shared by extraction and denial-of-service.",
      xp: 20,
    },
    ...
    // ── Analyst choice 2 ────────────────────────────────────────────────────
    {
      ...
      event: {
        ...
        raw: {
          ...
          "data.office365.ExchangeMetaData.From": "m.ferreira@meridianfintech.com",
          "data.office365.ExchangeMetaData.To": ["procurement@northbridge-client.com"],
          "data.office365.ExchangeMetaData.Subject": "Contract Renewal — Next Steps",
          "data.office365.ExchangeMetaData.AttachmentDetails.Name": "",
          "data.office365.ExchangeMetaData.RecipientCount": "1",
          "data.office365.ExchangeMetaData.SensitivityLabelNames": [],
        },
      },
