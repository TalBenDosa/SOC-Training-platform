  ...
  tasks: [
    ...
    // ── Reading 6: token theft two layers ────────────────────────────────────
    {
      ...
      content:
        ...
        "**Access Token Manipulation: Token Impersonation/Theft** is mapped by MITRE ATT&CK to the **Privilege Escalation** tactic (and, as of ATT&CK's 2026 restructuring of the old Defense Evasion tactic, also to the tactic now named **Stealth**, which inherited Defense Evasion's TA0005 ID — most existing tooling and documentation still labels it 'Defense Evasion'). It has nothing to do with a browser cookie or an IdP: it abuses Windows APIs (`DuplicateToken`, `DuplicateTokenEx`, `ImpersonateLoggedOnUser`) to copy an existing Windows security token already present on a compromised machine.\n\n" +
