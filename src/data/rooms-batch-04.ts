...
- **Agents** — list of all monitored endpoints with connectivity status, OS, last alert time`,
    ...
    // ----- Reading 4: ossec.conf, local_rules.xml, local_decoder.xml --------
    {
      type: "reading",
      id: "wazuh-r4",
      heading: "Configuring Wazuh: ossec.conf, local_rules.xml, and local_decoder.xml",
      content: `Everything covered so far — decoders, rules, severity levels, FIM, active response — is Wazuh's *behaviour*. That behaviour is not fixed at install time. It lives in plain-text configuration files, and knowing exactly which file to open (and which files to never touch) is the difference between an analyst who can read Wazuh and one who can actually run it.

**ossec.conf — the manager's central configuration file**

Every Wazuh manager and every Wazuh agent has its own \`ossec.conf\`, located at \`/var/ossec/etc/ossec.conf\`. It is a single XML document wrapped in a root \`<ossec_config>\` element, broken into blocks that each configure one subsystem:

- \`<global>\` — manager-wide behaviour: whether to write JSON alerts, whether to log every event or only alerts, email notification settings
- \`<ruleset>\` — tells the manager which directories to load decoders and rules from (the built-in ruleset directory, plus your custom directories)
- \`<localfile>\` — one block per log source the manager or agent should read: a file path and its format (syslog, json, eventchannel, full_command, and others)
- \`<integration>\` — external integrations such as VirusTotal lookups, Slack, PagerDuty, or a custom webhook
- \`<active-response>\` — binds a script to a rule ID, rule group, or severity level
- \`<syscheck>\` — File Integrity Monitoring configuration: which directories to watch and how often
- \`<rootcheck>\` — rootcheck / policy monitoring configuration

A trimmed but realistic manager \`ossec.conf\` looks like this:
...
<ossec_config>
  ...
  <localfile>
    <log_format>syslog</log_format>
    <location>/var/log/orderapp/app.log</location>
  </localfile>
</ossec_config>
\`\`\`

Notice the \`<ruleset>\` block loads decoders and rules from **two** separate directory pairs: \`ruleset/decoders\` + \`ruleset/rules\` (the vendor-shipped ones), and \`etc/decoders\` + \`etc/rules\` (yours). That separation is deliberate and it is the single most important operational fact in this reading.

**Never edit the shipped ruleset — write into local_rules.xml and local_decoder.xml instead**

The files under \`/var/ossec/ruleset/rules/\` and \`/var/ossec/ruleset/decoders/\` are the built-in detection content Wazuh ships with every release. When you upgrade Wazuh, those directories get overwritten. Anything you hand-edited inside them is silently gone the next time you patch.

Your own detection content belongs in exactly two files:
- \`/var/ossec/etc/rules/local_rules.xml\` — your custom rules
- \`/var/ossec/etc/decoders/local_decoder.xml\` — your custom decoders

These live under \`etc/\`, not \`ruleset/\`, so they survive every upgrade untouched. This is why rule ID 100000+ exists as a hard floor (covered in the previous reading) — it keeps your local IDs from ever colliding with a vendor ID that shifts between ruleset versions.

**Writing a custom rule in local_rules.xml**

Custom rules are almost always children of an existing built-in rule — you rarely start from a blank decoded event. Here is a rule that raises the severity of failed logins specifically against a sensitive service account, riding on top of the built-in sshd authentication-failure rule (5716):

\`\`\`
<group name="local,syslog,sshd,">
  <rule id="100010" level="10">
    <if_sid>5716</if_sid>
    <match>backup_admin</match>
    <description>Failed SSH login attempt against the backup_admin service account</description>
    <group>authentication_failures,</group>
  </rule>
</group>
\`\`\`

Read it top to bottom: \`<if_sid>5716</if_sid>\` means this rule is only even considered when built-in rule 5716 has already matched the same log line — it inherits 5716's decoding work rather than repeating it. \`<match>backup_admin</match>\` narrows that further to lines mentioning this one account. The result: an ordinary failed SSH login stays at 5716's default level, but a failed login against this specific automation account — which should never fail authentication under normal operation — is promoted to level 10, crossing the alerting threshold with a description that names exactly why it matters.

**Writing a custom decoder in local_decoder.xml**

Proprietary or in-house applications rarely ship with a Wazuh decoder, so you write one. Suppose an internal app writes lines like \`2025-06-24 10:15:22 ERROR user=jsmith action=login result=failed ip=10.0.5.22\` to \`/var/log/orderapp/app.log\`:

\`\`\`
<decoder name="orderapp">
  <program_name>orderapp</program_name>
</decoder>

<decoder name="orderapp-fields">
  <parent>orderapp</parent>
  <prematch>^\\S+ \\S+ \\S+ user=</prematch>
  <regex>user=(\\S+) action=(\\S+) result=(\\S+) ip=(\\S+)</regex>
  <order>dstuser, extra_data, status, srcip</order>
</decoder>
\`\`\`

The first decoder is the parent — it just identifies "this log came from orderapp." The second is a child decoder that runs only after the parent matches, using \`prematch\` to locate the start of the field block and \`regex\` with \`order\` to extract four values into named fields (\`data.dstuser\`, \`data.extra_data\`, \`data.status\`, \`data.srcip\`). Once those fields exist, you can write rules against them exactly like any built-in decoded field.

**Nothing takes effect until you restart**

Editing these files does not change live behaviour. The manager only reloads its ruleset on restart: \`systemctl restart wazuh-manager\`, or \`/var/ossec/bin/wazuh-control restart\`. Skipping this step is a common source of "I wrote the rule and nothing happened" confusion — the rule was saved to disk correctly, but the running process is still evaluating the ruleset it loaded at its last start.`,
      checkpoint: {
        question:
          "According to the reading, why do custom Wazuh rules and decoders belong in /var/ossec/etc/rules/local_rules.xml and /var/ossec/etc/decoders/local_decoder.xml rather than directly inside /var/ossec/ruleset/rules/ or /var/ossec/ruleset/decoders/?",
        options: [
          "The ruleset/ directories are read-only at the filesystem level and cannot be edited",
          "Files under ruleset/ are the vendor-shipped content and get overwritten on every Wazuh upgrade, while etc/ files persist",
          "Only the dashboard user, not the manager process, has permission to read ruleset/",
          "Rules under 100000 must be stored in etc/ for licensing reasons",
        ],
        answer: 1,
        explanation:
          "The ruleset/ directories hold Wazuh's built-in detection content and are overwritten whenever you upgrade the product, silently discarding any hand edits made there. The etc/ directory is never touched by upgrades, which is why local_rules.xml and local_decoder.xml — plus the 100000+ ID floor for custom rules — are the only safe place to build detection content that survives patching.",
      },
    } satisfies ReadingTask,

    // ----- Reading 5: wazuh-logtest -----------------------------------------
    {
      type: "reading",
      id: "wazuh-r5",
      heading: "wazuh-logtest: Validating a Rule Before It Ever Fires in Production",
      content: `Writing a rule is the easy part. The expensive mistake is deploying it straight to a live manager without ever checking what it actually does against real traffic — a rule that is slightly too broad floods the dashboard with false positives at 3 AM; a rule that is slightly too narrow means an entire category of attack silently produces zero alerts for months, and nobody notices until an incident review asks "didn't we have a detection for this?" Both failures are avoidable, and Wazuh ships a tool built specifically to avoid them: **wazuh-logtest**.

**What wazuh-logtest actually does**

\`wazuh-logtest\`, located at \`/var/ossec/bin/wazuh-logtest\`, is an interactive tool that runs the exact same decoding-and-rule-matching pipeline the manager uses in production — decoders, rules, active response bindings — but entirely offline. You paste in a single raw log line, and it shows you, step by step, exactly what the manager would have done with it, without generating a real alert, without touching the Indexer, and without any live agent connection required.

Run it and paste a sample SSH log line, and the output walks through three phases:

\`\`\`
**Phase 1: Completed pre-decoding.
    full event: 'Jun 24 14:32:11 srv01 sshd[2341]: Failed password for invalid user admin from 185.220.101.45 port 54321 ssh2'
    hostname: 'srv01'
    program_name: 'sshd'
...
- **Phase 1 (pre-decoding)** extracts the generic syslog envelope — timestamp, hostname, program name — before any application-specific parsing happens.
- **Phase 2 (decoding)** shows which decoder claimed the log and exactly which fields it pulled out. If your custom decoder's regex is wrong, this is where you see it immediately: the fields you expected simply will not be there, or a field will contain the wrong slice of text.
- **Phase 3 (filtering)** shows which rule, if any, matched the decoded fields, along with its level and description.

**The phase that matters most: when Phase 3 shows nothing**

If a log line makes it through Phase 1 and Phase 2 cleanly but Phase 3 produces no output at all, that is the single most important signal wazuh-logtest gives you: **no rule fired on this event.** Either your rule's conditions do not actually match the fields the decoder extracted, a typo in a \`<match>\` or \`<field>\` value means it will never trigger, or no rule — built-in or custom — covers this log pattern at all. Finding that out in a test session costs nothing. Finding it out three months into production, when an incident review discovers this exact attack pattern generated zero alerts the whole time, costs a great deal more.

**Testing before you touch production, every time**

The workflow that actually protects a SOC is: write the rule in local_rules.xml → restart the manager in a test environment (or a sandboxed instance, never straight in production) → run wazuh-logtest against both a sample of the malicious traffic you want to catch (it should produce your new rule ID) and a sample of ordinary legitimate traffic that superficially resembles it (it should produce nothing, or only the expected lower-severity parent rule). Only once both checks pass does the rule move to production. Some teams keep a small library of "known good" and "known bad" sample log lines specifically to re-run through wazuh-logtest every time a rule is tuned, catching regressions before they ever reach the dashboard. There is also a non-interactive mode (\`wazuh-logtest -q\`, piping a log file in) built for exactly this kind of repeatable, scriptable check.`,
      checkpoint: {
        question:
          "You run wazuh-logtest against a raw log line. Phase 1 and Phase 2 both complete and show correctly extracted fields, but Phase 3 produces no output at all. What does this tell you?",
        options: [
          "The manager is down and cannot be reached from the test tool",
          "The log matched a rule at severity level 0, which is hidden from the output by default",
          "No rule — built-in or custom — currently matches this decoded event, so no alert would be generated in production",
          "The log format is unsupported and must be converted to JSON before Wazuh can process it",
        ],
        answer: 2,
        explanation:
          "wazuh-logtest runs the manager's real pipeline locally with no live connection required, so a missing Phase 3 output simply means the decoded fields did not satisfy any active rule's conditions — the log would generate zero alerts if this same line arrived in production. That is exactly the gap you want to catch in testing, before an attack technique goes undetected for months.",
      },
    } satisfies ReadingTask,

    // ----- Reading 6: correlation rules --------------------------------------
    {
      type: "reading",
      id: "wazuh-r6",
      heading: "Correlation Rules: if_sid, if_matched_sid, same_field, different_field",
      content: `Every rule seen so far reacts to a single log line. Real attacks rarely announce themselves in one event — a brute force is dozens of failed logins followed by one success; a compromised account is a normal login followed by an abnormal action minutes later. Wazuh's correlation directives exist to connect those separate events into one higher-confidence alert, and the two building blocks that confuse people most are \`if_sid\` versus \`if_matched_sid\`, and \`same_field\` versus \`different_field\`.

**if_sid — same log line, parent-child hierarchy**

\`<if_sid>\` was already introduced in the previous reading: a rule tagged with \`<if_sid>5716</if_sid>\` is only evaluated when rule 5716 has already matched **the same decoded log line currently being processed**. It is not correlation across time — it is a shortcut that says "inherit the parent's decoding work and narrow further." Two events, ten minutes apart, never interact through \`if_sid\`.

**if_matched_sid — correlation across separate events, over time**

\`<if_matched_sid>\` is different in kind, not just in name. A rule using \`<if_matched_sid>100020</if_matched_sid>\` fires when rule 100020 matched on a **previous, distinct log line** within the memory the manager keeps of recently matched rules — this is genuine multi-event correlation. Combined with \`<frequency>\` and \`<timeframe>\`, you can require that a referenced rule matched N times within a time window before the correlation rule considers itself triggered.

**same_field and different_field — constraining correlation to a shared context**

On their own, \`if_matched_sid\` correlations are dangerously loose: "rule 100020 fired sometime in the last two minutes" says nothing about whether it fired for the *same attacker*. \`<same_field>\` fixes that by requiring a named field to hold the identical value across both correlated events — \`<same_field>srcip</same_field>\` means the current event's source IP must match the source IP of the event that satisfied \`if_matched_sid\`. (\`<same_source_ip/>\` is a shorthand for exactly this on the srcip field.) \`<different_field>\` does the opposite: it requires the field to change between the two events — useful for detecting things like the same account authenticating from two different source IPs in quick succession, where a *match* would be the uninteresting case and a *difference* is the signal.

**Worked example: brute force that succeeds**

Five failed SSH logins followed by a successful one, all from the same source, is far more dangerous than either half alone — it suggests the attacker eventually guessed or already held the right credential. Building it in two stages:

\`\`\`
<rule id="100020" level="10" frequency="5" timeframe="120">
  <if_matched_sid>5716</if_matched_sid>
  <same_source_ip />
  <description>5 SSH authentication failures from the same source within 2 minutes</description>
  <group>authentication_failures,</group>
</rule>
...
<rule id="100021" level="12">
  ...
  <same_source_ip />
  <description>Successful SSH login immediately following a brute-force burst from the same source</description>
...
Rule 100020 is the first correlation stage: it fires once built-in rule 5716 (authentication failed) has matched 5 times within 120 seconds from the same source IP — a burst, not just an occasional typo. Rule 100021 is the second stage: it only considers the current log if it is itself a successful login (\`if_sid\` against built-in rule 5715, "sshd authenticated") **and** rule 100020 fired previously (\`if_matched_sid\`) **and** the source IP matches across both (\`same_source_ip\`). Miss any one of those three conditions — a success from a different IP, or a success with no preceding burst — and 100021 stays silent. That precision is exactly why \`same_field\`/\`same_source_ip\` matters: without it, any unrelated successful login anywhere in the organisation within two minutes of any brute-force burst anywhere else would falsely correlate.

**Choosing between them**

The decision is really about scope: reach for \`if_sid\` when you are narrowing a single event that a parent rule already decoded, and reach for \`if_matched_sid\` (usually paired with \`same_field\`/\`same_source_ip\`, and often \`frequency\`/\`timeframe\`) the moment you need to connect two or more separate log lines into one story.`,
      checkpoint: {
        question:
          "According to the reading, what is the key difference between if_sid and if_matched_sid in a Wazuh rule?",
        options: [
          "if_sid is deprecated as of Wazuh 4.x and has been fully replaced by if_matched_sid, so every custom rule written today should reference if_matched_sid exclusively",
          "if_sid checks whether a rule matched on the same decoded log line being processed right now; if_matched_sid checks whether a rule matched on a separate, earlier log line within the manager's correlation memory",
          "if_sid only works when the underlying decoder was written for a Windows Event Log source, while if_matched_sid is restricted to rules built purely on Linux syslog decoders",
          "if_matched_sid only functions once an active-response script has already been bound to the rule it references, since the correlation depends on triggering that script's output",
        ],
        answer: 1,
        explanation:
          "if_sid is a same-event, parent-child shortcut — it never looks across time. if_matched_sid is genuine cross-event correlation, referencing a rule that matched on a distinct, earlier log line, which is why it is the directive used to connect a burst of failures to a later successful login rather than same_field or same_source_ip alone.",
      },
    } satisfies ReadingTask,

    // ----- Reading 7: operating Wazuh ----------------------------------------
    {
      type: "reading",
      id: "wazuh-r7",
      heading: "Operating Wazuh: agent_control, cluster_control, the REST API, and Agent Enrollment",
      content: `Reading alerts and writing detection content covers what Wazuh does. Day-to-day, a SOC also has to keep the platform itself running: knowing which agents are actually reporting, managing a multi-manager deployment at scale, and enrolling every new endpoint that gets built. Four tools cover that ground.

**agent_control — managing individual agents**

\`/var/ossec/bin/agent_control\` is the command-line tool for day-to-day agent management. The most common invocations:
- \`agent_control -l\` — list every registered agent with its ID, name, and connection status (Active, Disconnected, Never connected, Pending)
- \`agent_control -i <agent_id>\` — detailed info on a single agent (OS, IP, last keep-alive, agent version)
- \`agent_control -r -u <agent_id>\` — restart a specific agent remotely
- \`agent_control -R -u <agent_id>\` — remove an agent's key, deregistering it

The status column is the first thing to check when an alert looks "quiet" from a host you'd expect to be noisy: an agent showing **Disconnected** has stopped shipping logs entirely, which means an attacker who kills the agent process on a compromised endpoint can go dark from the SIEM's perspective without triggering a single new alert — silence itself is a signal worth its own monitoring.

**cluster_control — managing a multi-manager deployment**

A single Wazuh manager eventually becomes a bottleneck at scale, so larger deployments run a **cluster**: one master node plus one or more worker nodes, with agents distributed across workers and the master keeping ruleset and configuration synchronised to every worker. \`/var/ossec/bin/cluster_control\` is the tool for operating it:
- \`cluster_control -l\` — list every node in the cluster and its sync status
- \`cluster_control -i\` — general cluster health information
- \`cluster_control -a\` — list every agent together with which cluster node it is currently reporting to

This matters operationally because a rule change only takes effect once the master has finished propagating it to every worker — if you tune a rule and it seems to work on some hosts but not others, checking which worker each agent reports to (and whether that worker has finished syncing) is usually the first thing to check.

**The Wazuh REST API — automation beyond the dashboard**

Both the dashboard and \`agent_control\`/\`cluster_control\` ultimately talk to the same underlying interface: the Wazuh Manager's REST API, listening on TCP 55000 (introduced in the architecture reading) and secured with token-based authentication. Anything the dashboard can do, the API can do programmatically — \`GET /agents\` to list agents, \`PUT /agents/{agent_id}/restart\` to restart one remotely, \`GET /rules\` to search the currently loaded ruleset, \`PUT /active-response\` to trigger a response manually. This is the integration point SOAR platforms, ticketing systems, and custom automation scripts use to act on Wazuh data without a human clicking through the dashboard — an automated playbook that isolates a host the moment a critical rule fires is built on this API, not on the UI.

**Agent enrollment — getting a new endpoint reporting**

A freshly built server does not automatically appear in Wazuh; it has to be enrolled, which comes down to both sides agreeing on a shared key. There are two ways to get there:

- **Manual key exchange**: on the manager, \`manage_agents\` (also at \`/var/ossec/bin/\`) registers the new agent (assigning it an ID and name) and generates a unique key; that key is copied to the new endpoint and imported with \`manage_agents\` there too, before the agent service is started. Fine for a handful of servers, painful at scale.
- **Automatic enrollment via agent-auth**: the manager's \`authd\` service listens on **TCP port 1515** — a separate port from the 1514 used for ongoing data traffic — dedicated purely to enrollment. The new endpoint runs \`agent-auth -m <manager_ip>\`, which requests a key over 1515 and receives it automatically, no manual copy-paste required. This is what makes Wazuh practical to bake into a golden image or a Terraform/Ansible provisioning pipeline: every new VM enrolls itself on first boot.

Either way, once the key exchange completes and the agent service starts, the new endpoint should appear in \`agent_control -l\` with status **Active** within seconds — that is the first place to check when an enrollment "doesn't seem to have worked."`,
      checkpoint: {
        question:
          "According to the reading, which TCP port does the Wazuh manager's authd service use for automatic agent enrollment, distinct from the port used for ongoing log data?",
        options: ["TCP 1514", "TCP 1515", "TCP 55000", "TCP 443"],
        answer: 1,
        explanation:
          "TCP 1515 is dedicated to agent enrollment via authd — separate from TCP 1514, which carries ongoing encrypted log/event traffic between an already-enrolled agent and the manager, and separate from TCP 55000, which is the REST API port.",
      },
    } satisfies ReadingTask,
    ...
    // ----- Question 4 --------------------------------------------------------
    {
      ...
      id: "wazuh-q4",
      question:
        "You want a rule that fires only when the same source IP behind a burst of 5 failed SSH logins (custom rule 100020, using frequency and timeframe) goes on to achieve a successful login within the next two minutes. Which combination of directives builds this correctly on the new rule?",
      options: [
        "<if_sid>100020</if_sid> alone — if_sid already checks whether a rule matched at any point in the recent past",
        "<if_matched_sid>100020</if_matched_sid> combined with <same_source_ip /> — correlating a prior match of rule 100020 with an identical source IP field on the current event",
        "<different_field>srcip</different_field> alone, so the rule fires whenever the source IP changes between the two events",
        "<frequency>1</frequency> with no if_sid or if_matched_sid directive, since frequency alone can reference a different rule ID's previous matches",
      ],
      answer: 1,
      explanation:
        "if_matched_sid is the directive for cross-event correlation — it looks back at whether a referenced rule matched on a separate, earlier log line, which is exactly what's needed to connect a brute-force burst to a later successful login. if_sid only evaluates the current, single decoded log line, so it can never see an earlier event and would leave this rule permanently silent. same_source_ip is required alongside it — without constraining the two correlated events to the same source IP, the rule would false-positive on any unrelated successful login anywhere in the organisation that happened to follow any brute-force burst within the timeframe. different_field would require the IP to change, which is the wrong direction for confirming it's the same attacker, and frequency without an if_sid/if_matched_sid anchor only counts repeats of the rule it's attached to — it cannot reference a separate rule ID at all.",
      xp: 25,
    } satisfies QuestionTask,
    ...
    // ----- Question 5 --------------------------------------------------------
    {
      ...
      id: "wazuh-q5",
      question:
        "A junior analyst writes a new custom rule in local_rules.xml intended to catch a specific web-shell upload pattern in Apache logs, and pushes it straight to the production manager without testing it first. Two days later, a legitimate nightly reporting job that happens to hit a similar URL pattern floods the dashboard with false positives. What would running wazuh-logtest before deployment most likely have caught?",
      options: [
        "Nothing — wazuh-logtest can only evaluate Wazuh's built-in rules, not custom ones added to local_rules.xml",
        "Running the new rule's conditions against a sample of the legitimate reporting job's log lines would have shown Phase 3 matching when it shouldn't, revealing the rule was broader than intended before it ever touched production",
        "wazuh-logtest only verifies network connectivity between an agent and the manager, not rule logic",
        "wazuh-logtest requires a live, currently-connected agent to test any rule, so it could not have been used before the endpoint existed",
      ],
      answer: 1,
      explanation:
        "wazuh-logtest runs the manager's real decoding-and-rule-matching pipeline entirely offline against sample log lines, custom rules included. Feeding it a sample of the legitimate reporting job's Apache log lines alongside the intended malicious pattern would have shown the new rule matching (Phase 3 producing output) on traffic it should have ignored — exactly the over-broad match condition that caused the false-positive storm — with zero blast radius, since testing never touches the live Indexer or generates real alerts. It works fully offline and does not require any live agent connection.",
      xp: 25,
    } satisfies QuestionTask,
  ],
};
